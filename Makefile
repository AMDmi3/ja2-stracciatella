...
install: $(BINARY)
	...
	test -z "$(INSTALLABLE)" || install -d $(DESTDIR)$(SHARED_DIR)/applications
	test -z "$(INSTALLABLE)" || install -d $(DESTDIR)$(SHARED_DIR)/icons/$(ICON_THEME)/scalable/apps
	...
	test -z "$(INSTALLABLE)" || cp _build/distr-files-linux/*.desktop $(DESTDIR)$(SHARED_DIR)/applications
	test -z "$(INSTALLABLE)" || cp _build/icons/logo.svg $(DESTDIR)$(SHARED_DIR)/icons/$(ICON_THEME)/scalable/apps/ja2-stracciatella.svg
